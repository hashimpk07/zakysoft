...
class OrderCancellationStatisticsPerHour implements Graph
{
    public function data($request = null)
    {
        ...
        //$labels = array_map(fn($hour) => str_pad($hour, 2, '0', STR_PAD_LEFT) . ":00", range(6, 5, 23)); // Labels for 24 hours (6 AM to 5 AM next day)
        ...
        if ($quadrant) {
            ...
            //         $key = $regionId . '_' . $hour;
            //         $cancelledOrders[$hour] += $data[$key]->cancelled_orders ?? 0;
            //     }
            // }

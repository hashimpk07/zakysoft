...
class ManagementClientOperationalClientsByRegion implements Graph
{
    public function data($request = null)
    {
        ...
        foreach ($quadrants as $quadrant) {
            ...
            $count    = $data[$quadrant->id]->client_count ?? 0;
            $values[] = $count;
            $total += $count;
        }
        ...
        if (isset($data['-1'])) {
            ...
            $values[] = $data['-1']->client_count ?? 0;
            $total += $data['-1']->client_count ?? 0;
        }

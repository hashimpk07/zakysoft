...
class ManagementThirdPartyActiveCaptainByVehicleType implements Graph
{
    public function data()
    {
        ...
        foreach ($vehicle_types as $type) {
            $captainsQuery = Captain::belongsToMe()->excludeQuadrants()->active()->when($company, function ($query, $company) {
                $query->whereHas('captainThirdParty', function ($query) use ($company) {
                    $query->where('third_party_logistic_company_id', '=', $company)->excludeCompanies();
                });

...
class ThirdPartyCaptainPerformanceExportJob extends QueueExport
{
    ...
    public function getReport()
    {
        ...
        $from_date       = isset($request['from_date']) ? Carbon::parse($request['from_date'])->format('Y-m-d 00:00:00') : Carbon::now()->startOfMonth("Y-m-d 00:00:00");
        ...
        return CaptainWorkingLog::query()
            ...
            ->when($companies, function ($query, $companies) {
                return $query->whereHas('captain.company', function ($query) use ($companies) {
                    $query->where('third_party_logistic_companies.id', $companies);
                });
            })

...
class HighLevelReportExport extends QueueExport
{
    protected int $chunk = 2000;
    protected string $file_name = 'high-level-report';
    ...
    protected function getFieldValue($field, $record): string
    {
        switch ($field) {
            ...
            case 'cancelled_by':
                return $record->order_status_id === OrderStatus::DELIVERED
                    ...
                    : ($record->lastLog && $record->lastLog->canceled_by
                        ...
                        : 'By System');
            case 'assigned_by':
                return $record->orderReport && $record->orderReport->assignedBy 
                    ? $record->orderReport->assigned() 
                    : ($record->assigned_by ?? '');
            case 'created_at':
            ...
            case 'is_relocated':
                return (string) ($record->relocated_count ?? '');
            case 'relocation_history':
                return $record->relocation_history ?? 'N/A';
            case 'arrival_time':
    ...
    {
        $selectedFields = $this->export->filters['selected_fields'] ?? [];
        $allFields = ReportFieldsService::getFields('high_level_report');
    ...
    public function getData()
    {
        ...
        if (strlen($fromTime) === 5) { // HH:MM → add seconds
            $fromTime .= ':00';
        }

        $fromDate = isset($request['from_date'])
            ? Carbon::parse($request['from_date'] . ' ' . $fromTime)
            : now()->subDays(6)->setTimeFromTimeString('06:00:00');

        // TO DATE
        $toTime = $request['order_time_to'] ?? '05:59:59';
        if (strlen($toTime) === 5) { // HH:MM → add seconds
            $toTime .= ':59';
        }

        $toDate = isset($request['to_date'])
            ? Carbon::parse($request['to_date'] . ' ' . $toTime)->addDay()
            : now()->addDay()->setTimeFromTimeString('05:59:59');
